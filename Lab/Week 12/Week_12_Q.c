...
void smallestValue2(BTNode *node, int *m){
    if (node == NULL)  	return;

    smallestValue2 (node->left, m); //Visit the left child node
    
    smallestValue2(node->right, m);//Visit the right child node
    if (node->item< *m) *m=node->item;    
}
