...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
    int count = ll1->size; /* initialise counter */
    int i;
    int n =1;              /* odd number alternating seed */

    /* if ll2 is empty return no insertion made */
    if (ll2 == NULL)
    {
        return;
    }

    ListNode *cur = ll2->head; /* initialise pointer */

    /* insert node at every alternating position */
    for (i = 0; i < count; ++i) 
    {
        insertNode(ll1,2*n-1,cur->item);    /* insert node at every alternating position */
        n++;    /* n act as an alternating series */
        cur = cur->next;
        removeNode(ll2,0); /* remove the node after insertion */
        if (cur == NULL)    /* when ll2 is empty break out the loop */
        {                   /* even if for loop is not finished     */
            break;      
        }
    }
}
